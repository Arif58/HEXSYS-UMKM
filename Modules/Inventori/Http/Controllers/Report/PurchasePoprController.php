...
class PurchasePoprController extends Controller{
    private $folder_path = 'report.purchase-popr';
    ...
    function index($id = NULL){
		$pageFileName  = 'index';
		$title         = 'Laporan Permohonan Pembelian';
	...
	function realisasi($id = NULL){
		...
		$title         = 'Realisasi Permohonan Pembelian';
    ...
    function getData(Request $request){
		...
		->where(function($where) use($request){
			...
			$where->where('popr_st','1');
			//$where->whereRaw("coalesce(popr_st,'')='1'");
		})
