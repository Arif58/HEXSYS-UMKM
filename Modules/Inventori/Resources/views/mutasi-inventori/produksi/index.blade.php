	...
	<div class="card">
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="form-group form-group-float">
                        <input name="prod_no_param" id="prod_no_param" placeholder="Pencarian Nomor Produksi" class="form-control" data-fouc />
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group form-group-float">
            ...
            <button type="button" class="btn btn-primary legitRipple" id="tambah"><i class="icon-add mr-2"></i> Buat Produksi</button>
            <!--<button type="button" class="btn btn-info legitRipple" id="detail"><i class="icon-menu-open mr-2"></i> Detail Produksi</button>
            ...
            <div class="table-responsive">
                <table class="table datatable-pagination" id="tabel-data" width="100%">
                    <thead>
                        <tr>
                            <th id="prod_cd_table">prod_cd_table</th>
                            <th id="prod_no_table">Nomor</th>
                            <th id="pos_cd_table">pos_cd_table</th>
                            <th id="pos_nm_table">Gudang</th>
                            <th id="trx_date_table">Tanggal</th>
							<th id="prod_item_table">Barang Produksi</th>
							<th id="prod_item_nm_table">Barang Produksi</th>
							<th id="quantity_table">Jumlah</th>
                            <th id="prod_st_table">prod_st_table</th>
                            <th id="prod_st_nm_table">Status</th>
...
<script>
    ...
    $(document).ready(function(){
        ...
        tabelData = $('#tabel-data').DataTable({
			...
			dom 			: 'Blrtip',
            columns: [
                { data: 'prod_cd', name:'prod_cd', visible:false},
                { data: 'prod_no', name:'prod_no', visible:true},
				...
				{ data: 'trx_date', name:'trx_date', 
					...
					visible:true},
				{ data: 'prod_item', name:'prod_item', visible:false},
                { data: 'prod_item_nm', name:'prod_item_nm', visible:true},
				{ data: 'quantity', name:'quantity', defaultValue : '',visible:true, render: $.fn.dataTable.render.number( '.', ',', 0, '' )},
                { data: 'prod_st', name:'prod_st', visible:false},
                { data: 'prod_st_nm', name:'prod_st_nm', visible:false},
        ...
        $(document).on('keyup', '#prod_no_param',function(){ 
            tabelData.column('#prod_no_table').search($(this).val()).draw();
        });
        ...
        $('#reload-table').click(function(){
			setDaterange();
			$('input[name=prod_no_param]').val('').trigger('keyup');
			tabelData.ajax.reload();
		});
        ...
        $(document).on('click', '.hapus',function(){
			var rowData = tabelData.row($(this).parents('tr')).data();
			dataCd = rowData.prod_cd;
			dataSt = rowData.prod_st;
				
			/* if (rowData.prod_st != 'INV_TRX_ST_1') {
                swal({
                        ...
                        $.ajax({
                            ...
                            success: function(response)
                            {
                                if (response.status == 'ok') {
									...
									window.location = "{{ url('inventori/mutasi-inventori/produksi/') }}";
                                }else{
