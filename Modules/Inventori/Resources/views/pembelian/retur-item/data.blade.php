...
@section('content')
	...
	<div class="card">
        ...
        <div class="card-body">
            <form class="form-validate-jquery" id="form-isian" method="POST" action="{{ url('inventori/pembelian/retur-item/') }}">
                @csrf
                <div class="row">
                    <div class="col-md-4">
                        <label class="form-group-float-label is-visible">Nomor</label>
                        <div class="form-group form-group-float">
                            {{--<input name="retur_no" id="retur_no" class="form-control" required data-fouc value="{{ $defaultNo }}" placeholder="Kosong : nomor akan di-generate sistem" />--}}
							<input name="retur_no" id="retur_no" class="form-control" data-fouc placeholder="Kosong : nomor akan di-generate sistem" />
                        </div>
                    </div>
					<!--
                    <div class="col-md-4">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group form-group-float">
                            <input type="number" name="percent_ppn" id="percent_ppn" class="form-control" required data-fouc value="0" min="0"/>
                        </div>
                    </div>
            ...
            @if ($retur) 
                @if($retur->retur_st == 'INV_TRX_ST_1')
                    <form class="form-validate-jquery" id="form-item">
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <label class="form-group-float-label is-visible">Nama Barang</label>
                                <div class="form-group form-group-float">
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="form-group form-group-float">
                                    <input type="number" name="unit_price" id="unit_price" class="form-control money" required data-fouc value="0"/>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-end align-items-center">
                            <button type="button" id="load-faktur" class="btn btn-info ml-3 legitRipple"><i class="icon-folder-open2"></i> Load Faktur</button>
                ...
                <div class="table-responsive">
                    <table class="table datatable-pagination" id="tabel-data" width="100%">
                        <thead>
                            <tr>
                                <th id="po_retur_detail_id_table">po_retur_detail_id_table</th>
        ...
        <div class="card-footer">
            @if(!empty($retur))
                @if($retur->retur_st == 'INV_TRX_ST_1')
                    <button type="button" class="btn btn-warning legitRipple" id="proses"><i class="icon-check mr-2"></i>Proses Retur</button>
                    <button type="button" class="btn btn-danger legitRipple" id="hapus"><i class="icon-trash mr-2"></i>Hapus Retur</button>
                @endif
                    <button type="button" class="btn btn-success legitRipple" id="print"><i class="icon-printer mr-2"></i>Cetak Retur</button>
            @endif
            <button type="reset" class="btn btn-light legitRipple" id="reset"><i class="icon-reload-alt mr-2"></i> Kembali </button>
        </div>
    </div>
    <div class="modal fade" id="modal-load-faktur">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Load Faktur</h4>
                ...
                <div class="modal-body" >
                    <div class="table-responsive">
                        <table class="table table-single-select datatable-pagination" id="tabel-retur-kiri" style="width: 100%">
                        <!-- <table class="table table-single-select datatable-pagination" id="tabel-retur-kiri" width="100%"> -->
                            <thead>
                                <tr>
                                    <th>Tanggal</th>
                                    <th>No.PO</th>
...
<script>
    ...
    $(document).ready(function(){
        ...
        @if($retur)
            dataCdRetur  = "{{ $retur->retur_cd }}";
            $('select[name=supplier_cd]').val("{{ $retur->supplier_cd }}").trigger('change').attr("disabled", true);
            $('select[name=principal_cd]').val("{{ $retur->principal_cd }}").trigger('change').attr("disabled", true);

            $('input[name=retur_no]').val("{{ $retur->retur_no }}").trigger('keyup').attr("readonly", true);
            
			//$('input[name=trx_date]').val("{{ $retur->trx_date }}").trigger('keyup').attr("readonly", true);
			$('input[name=trx_date]').val("{{ date_format(date_create($retur->trx_date), "d/m/Y") }}").trigger('keyup').attr("readonly", false);
			
            $('input[name=percent_ppn]').val("{{ $retur->percent_ppn }}").trigger('keyup').attr("readonly", true);
            $('textarea[name=note]').val('{{ $retur->note }}').prop("readonly", true);
            $('input[name=ppn]').val('Rp '+spelling('{{ $retur->ppn }}')).attr('readonly', true);  
            $('input[name=total_amount]').val('Rp '+spelling('{{ $retur->total_amount }}')).attr('readonly', true); 
            ...
            tabelData = $('#tabel-data').DataTable({
                ...
                dom : 'tpi',
                columns: [
                    {name: "po_retur_detail_id", data: "po_retur_detail_id", visible:false},
            ...
            @if($retur->retur_st == 'INV_TRX_ST_1')
                function myCallbackFunction (updatedCell, updatedRow, oldValue) {
                    ...
                    $.ajax({
                        url : baseUrl+'/'+'item/'+rowData['po_retur_detail_id'],
                        type: "PUT",
                        dataType: "JSON",
                        data: {
                        ...
                        success: function(response)
                        {
                            if(response["status"] == 'ok') { 
                                ...
                                swal({title: "Update Retur Gagal",type: "error",showCancelButton: false,showConfirmButton: false,timer: 1000});
                            }
                        },
                ...
                $('#form-item').submit(function(e){
                    ...
                    // handle the invalid form...
                        ...
                        var url     = baseUrl+'/item/'+dataCdRetur;
                        var method  = 'POST';
                ...
                $(document).on('click', '#proses',function(){ 
                    swal({
                        ...
                        if(result.value){
                            swal({allowOutsideClick : false, title: "Memproses Retur Item",onOpen: () => {swal.showLoading();}});
                            
                            $.ajax({
                                url : baseUrl+'/'+dataCdRetur,
                                type: "PUT",
                                dataType: "JSON",
                                data: {
                                    '_token': $('input[name=_token]').val(),
                                ...
                                success: function(response)
                                {
                                    if (response.status == 'ok') {
                                        swal({
                                            ...
                                            swal.close();
                                            window.location = baseUrl+'/'+dataCdRetur;
                                        });
                                    }else{
                                        swal({title: "Proses Retur Item Gagal",type: "error",showCancelButton: false,showConfirmButton: false,timer: 1000});
                                    }
                                },
                                error: function (jqXHR, textStatus, errorThrown)
                                {
                ...
                $(document).on('click', '#hapus',function(){
                    swal({
                        ...
                        if(result.value){
                            swal({allowOutsideClick : false, title: "Menghapus Retur Item",onOpen: () => {swal.showLoading();}});
                            ...
                            $.ajax({
                                ...
                                success: function(response)
                                {
                                    if (response.status == 'ok') {
                                        ...
                                        swal({title: "Retur Item Gagal Dihapus",type: "error",showCancelButton: false,showConfirmButton: false,timer: 1000});
                                    }
                                },
                                error: function (jqXHR, textStatus, errorThrown)
                                {
            ...
            @else
                ...
                /* $(document).on('click', '#print',function(){
                    window.open(baseUrl+'/print/'+dataCdRetur,'_blank');
                }); */
            @endif
        @endif
        ...
        /*load faktur */
        $(document).on('click', '#load-faktur',function(){
            $('#tabel-retur-kiri').DataTable().destroy();

                /*tabel retur kiri = asal */
            var oTable = $('#tabel-retur-kiri').DataTable({
                language: {
                    paginate: {'next': $('html').attr('dir') == 'rtl' ? 'Next &larr;' : 'Next &rarr;', 'previous': $('html').attr('dir') == 'rtl' ? '&rarr; Prev' : '&larr; Prev'}
                },
                pagingType: "simple",
                ...
                columns: [
                    ...
                    {name: "ri_no", data: "ri_no", visible:false},
                    {name: "item_cd", data: "item_cd", visible:false},
                    {name: "item_nm", data: "item_nm", visible:true},
            ...
            $('#modal-load-faktur').modal('show');
        });
        /*aktivitas moving PO */
        $('.btn-move').click(function() {
			oTable 		= $('#tabel-retur-kiri').dataTable();
			tableright 	= $('#tabel-retur-kanan').dataTable();
        ...
        $('.btn-tambah').click(function() {
            ...
            var record = {
                ...
                'faktur_no'         : rowData['faktur_no'],
                'faktur_date'       : rowData['faktur_date'],
                'note'              : rowData['note'],
		...
		/* $('.btn-move-down').click(function() {
            // alert('root');
		  	var tableright 	= $('#tabel-retur-kanan').DataTable();
		  	var o = tableright.rows().data();
		 	// var table_retur 	= $('#tabel-data').DataTable();
	        if( !!o.length ){
                // alert(o.length);
	            for(var i = 0; i < o.length; i++){	
	            // alert(tableright.column(0).data()[i]);	
		 	 	tabelData.row.add({
		 	 		'po_retur_detail_id' : tableright.column(1).data()[i],
					'item_cd' 		: tableright.column(2).data()[i],
					'item_nm'		: tableright.column(3).data()[i],
					'unit_cd'    	: tableright.column(4).data()[i],
					'unit_nm'    	: tableright.column(5).data()[i],
					'quantity'     	: tableright.column(6).data()[i],
					'unit_price'     	: tableright.column(7).data()[i],
					'trx_amount'	: 0,
					'faktur_no' 	: '',
					'faktur_date'	: '',
