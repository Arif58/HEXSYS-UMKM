...
@section('content')
	...
	<div class="card">
        ...
        <div class="card-body">
            ...
            <!--<button type="button" class="btn btn-primary legitRipple" id="konversi-inventori"><i class="icon-add mr-2"></i> {{ $title }}</button>
            <button type="button" class="btn btn-success legitRipple" id="konversi-multi"><i class="icon-spinner4 mr-2"></i> Multi Satuan</button>-->
			<button type="button" class="btn btn-primary legitRipple" id="konversi-multi"><i class="icon-spinner4 mr-2"></i> {{ $title }}</button>
                ...
                <div class="modal-body">
                    <form class="form-validate-jquery" id="form-isian" action="#">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group form-group-float">
                                    ...
                                    <input type="text" name="item_nm" id="item_nm" class="form-control text-uppercase" required="" placeholder="" aria-invalid="false" readonly>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group form-group-float">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group form-group-float">
                                    <label class="form-group-float-label is-visible">Gudang Tujuan Transfer</label>
                                    <select name="pos_cd_destination" id="pos_cd_destination" class="form-control form-control-select2 select-search" required data-fouc>
                                        <option value="">=== Pilih Gudang ===</option>
                                        @foreach ($gudangs as $item)
                                        <option value="{{ $item->pos_cd }}">{{ $item->pos_nm }}</option>                                                        
                                        @endforeach
                            ...
                            <div class="col-md-6">
                                <div class="form-group form-group-float">
                                    <label class="form-group-float-label is-visible">Konversi Ke</label>
                                    <select name="target_item_cd" id="target_item_cd" class="form-control form-control-select2 select-search" required data-fouc>
    ...
    <!-- /modal-stock-->
    <!-- modal-multi -->
	<div id="modal-multi" class="modal fade"  tabindex="-1" data-backdrop="false">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Konversi Multi Satuan</h5>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
                <input type="hidden" name="max_konversi" id="max_konversi" >
                <div class="modal-body">
                    <div class="row">
						<div class="col-md-6">
							<div class="form-group form-group-float">
								<label class="form-group-float-label is-visible">Nama Inventori</label>
								<input type="text" name="item_nm_konversi" id="item_nm_konversi" class="form-control text-uppercase" required="" placeholder="" aria-invalid="false" readonly>
							</div>
						</div>
						<div class="col-md-6">
							<div class="form-group form-group-float">
								<label class="form-group-float-label is-visible">Stok Inventori</label>
								<input type="text" name="stock_inventori_konversi" id="stock_inventori_konversi" class="form-control" readonly aria-invalid="false">
							</div>
						</div>
                        <div class="col-md-6">
                            <div class="form-group form-group-float">
                                <input type="text" name="status_konversi" id="status_konversi" class="form-control text-uppercase" readonly>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group form-group-float">
                                <label class="form-group-float-label is-visible">Jumlah Konversi</label>
                                <input type="number" name="jumlah_konversi" id="jumlah_konversi" min="1" class="form-control" aria-invalid="false">
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="reset" class="btn btn-light legitRipple" id="reset" data-dismiss="modal">Reset <i class="icon-reload-alt ml-2"></i></button>
                    <button type="button" id="submit-multi" class="btn btn-primary ml-3 legitRipple">Konversi <i class="icon-floppy-disk ml-2"></i></button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    var tabelData;
    var baseUrl     = "{{ url('inventori/mutasi-inventori/konversi/') }}";
    ...
    $(document).ready(function(){
        ...
        //biar select2 di dalem modal bisa dipake nyari
        $("#pos_cd_destination").select2({
            dropdownParent: $("#modal-stock")
        });
        $("#target_item_cd").select2({
            dropdownParent: $("#modal-stock")
        });
        //end
        ...
        $('#submit-button').click(function(){
            var stok = parseInt(($('input[name=stock_inventori]').val()).split(" ")[0]);
            var keluar = $('#jumlah_trx').val();
            if(keluar > stok){
                swal({
                    title: "Permintaan Konversi Melebihi Stok!",
                    type: "warning",
        ...
        /* transaksi */
        $('#konversi-inventori').click(function() {
            if (dataCd == null) {
                swal({
                    title: "Pilih Data!",
                    type: "warning",
                        ...
                        $.ajax({
                            ...
                            'success': function(response){
                                if(response["status"] == 'ok') {     
                                    ...
                                    swal({title: "Data Gagal Disimpan",type: "error",text:response['message'],showCancelButton: false,showConfirmButton: false,timer: 1000});
                                }
                            },
        ...
        $('#konversi-multi').click(function() {
            if (dataCd == null) {
                ...
                $.ajax({
                    'type': 'POST',
                    'url' : '{{ url("/inventori/daftar-inventori/cek-satuan") }}',
                    'data': {
                        '_token'    : $('meta[name="csrf-token"]').attr('content'),
                    ...
                    'success': function(response){
                        if(response.status == 'ok') {     
                            $('#status_konversi').val(response.message);
                            $('#max_konversi').val(Math.floor(rowData['quantity']));
							$('#item_nm_konversi').val(rowData['item_nm']);
							$('#stock_inventori_konversi').val(Math.floor(rowData['quantity']));
                            $('#jumlah_konversi').attr({
                                "max" : Math.floor(rowData['quantity']),
                            })
							$('#modal-multi').modal('show');
                            swal.close();
                        }else{
                            swal({title: "",type: "error",text:response.message,showCancelButton: false,showConfirmButton: false,timer: 1000});
                        }
                    },
                    'error': function(response){ 
                        var errorText = '';
        ...
        /*submit konversi multi satuan */
        $('#submit-multi').click(function() {
				//if(parseFloat($('#jumlah_konversi').val()) > parseFloat($('#max_konversi').val())){
				if(parseFloat($('#jumlah_konversi').val()) > parseFloat($('#stock_inventori_konversi').val())){
                    swal({
                        title: "Konversi Melebihi Stok!",
                        type: "error",
                        showCancelButton: false,
                        showConfirmButton: false,
                        timer: 1000
                    ...
                    var dataKonversi = {
                        '_token'    : $('meta[name="csrf-token"]').attr('content'),
                        'positemunit_cd'  : dataCd,
                        'jumlah_konversi' : $('#jumlah_konversi').val(),
                    };
                    console.log(dataKonversi);
                    swal({
                        title               : 'Simpan Data?',
                        type                : "question",
                        showCancelButton    : true,
