...
<script>
    var tabelData;
	var baseUrl     = "{{ url('inventori/report/stok-in/') }}";
    ...
    $(document).ready(function(){
        ...
        tabelData = $('#tabel-data').DataTable({
			...
			buttons: [
				...
				},
				{
					...
					customize: function (doc) {
						...
						var reportTitle =  'Laporan Stok In-Out ' + gudangSelect + ' | ' + tanggalSelect;	
												
						//--Full width table
						//doc.content[1].table.widths =Array(doc.content[1].table.body[0].length + 1).join('*').split('');
						doc.content[1].table.widths = [150,60,50,50,50,130];
            ...
            columns: [
                ...
                { data: 'move_tp', name: 'move_tp', visible:false },
				...
				{ data: null, name:'jumlah_trx', 
					"render": function ( data, type, row ) {
						if (data.move_tp == 'MOVE_TP_1') {
							return data.jumlah_trx;
						} else {
							return 0;
						}	
					}, 
					visible:true, className: "text-right"},
				{ data: null, name:'jumlah_trx', 
					"render": function ( data, type, row ) {
						if (data.move_tp == 'MOVE_TP_2') {
							return data.jumlah_trx;
						} else {
							return 0;
						}
					...
					visible:true, className: "text-right"},	
				{ data: "stok_awal", name : "stok_awal", visible:false, render: $.fn.dataTable.render.number( '.', ',', 2, '' ), className: "text-right" },
                //{ data: "stok_akhir", name : "stok_akhir", visible:true, render: $.fn.dataTable.render.number( '.', ',', 2, '' ), className: "text-right" },
				{ data: "stok_akhir", name : "stok_akhir", visible:true, render: $.fn.dataTable.render.number( ',', '.', 2, '' ), className: "text-right" },
