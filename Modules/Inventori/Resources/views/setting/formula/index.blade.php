            ...
            <div id="bagian-tabel">
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group form-group-float">
                            ...
                            <input name="search_param" id="search_param" placeholder="Pencarian Nama Formula" class="form-control" data-fouc />
                        </div>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-single-select datatable-pagination" id="tabel-data" width="100%">
                        <thead>
                            <tr>
                                <th width="15%">Kode Formula</th>
                                <th id="formula_nm_table" width="15%">Nama Formula</th>
            ...
            <div id="bagian-form">
                <form class="form-validate-jquery" id="form-isian" action="#">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group form-group-float">
                                <label class="form-group-float-label is-visible">Kode Formula</label>
                                <input type="text" name="formula_cd" class="form-control text-uppercase" required="" placeholder="" aria-invalid="false">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group form-group-float">
                                <label class="form-group-float-label is-visible">Nama Formula</label>
                                <input type="text" name="formula_nm" class="form-control" required="" placeholder="" aria-invalid="false">
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    var baseUrl     = "{{ url('inventori/setting/formula/') }}";
    ...
    $(document).ready(function(){
        ...
        tabelData = $('#tabel-data').DataTable({
            ...
            columns: [
                ...
                { data: 'formula_nm', name: 'formula_nm',visible:true },
            ],
        });

        $(document).on('keyup', '#search_param',function(){ 
            tabelData.column('#formula_nm_table').search($(this).val()).draw();
        });
        ...
        $(document).on('click', '#ubah',function(){ 
            if (dataCd == null) {
                ...
                $('input[name=formula_cd]').val(rowData['formula_cd']).prop('readonly',true);
                $('input[name=formula_nm]').val(rowData['formula_nm']);
                $('#bagian-tabel').hide();      
                $('#bagian-form').show(); 
            }
        });
        ...
        /*cek kode*/
        $('input[name=formula_cd]').focusout(function(){
            var id          = $(this).val();
                ...
                $.getJSON( urlUpdate, function(data){
                    if (data['status'] == 'ok') {
                        swal({
                            ...
                            $('input[name=formula_cd]').val('');
                            $('input[name=formula_cd]').focus();
                            swal.close();
                        });
                    }
                });
    ...
    function reset(type) {
        ...
        $('#bagian-tabel').show();      
        $('#bagian-form').hide(); 
        $('input[name=formula_cd]').val('').prop('readonly',false);
        $('input[name=formula_nm]').val('');
