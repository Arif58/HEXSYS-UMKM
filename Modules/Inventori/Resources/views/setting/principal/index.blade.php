                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group form-group-float">
                            {{-- <label class="form-group-float-label is-visible">Nama Principal</label> --}}
                            <input name="search_param" id="search_param" placeholder="Pencarian Nama Principal" class="form-control" data-fouc />
                        </div>
                    </div>
                </div>
                    ...
                    <table class="table table-single-select datatable-pagination" id="tabel-data" width="100%">
                        <thead>
                            <tr>
                                ...
                                <th id="principal_nm_table">Nama Principal</th>
                            </tr>
                        </thead>
                        <tbody>
                        </tbody>
            ...
            <div id="bagian-form">
                <form class="form-validate-jquery" id="form-isian" action="#">
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group form-group-float">
                                <label class="form-group-float-label is-visible">Kode Principal</label>
                                <input type="text" name="principal_cd" class="form-control text-uppercase" required="" placeholder="" aria-invalid="false">
                            </div>
                        </div>
                        <div class="col-md-8">
                            <div class="form-group form-group-float">
                                <label class="form-group-float-label is-visible">Nama Principal</label>
                                <input type="text" name="principal_nm" class="form-control" required="" placeholder="" aria-invalid="false">
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    var baseUrl     = "{{ url('inventori/setting/principal/') }}";
    ...
    $(document).ready(function(){
        ...
        tabelData = $('#tabel-data').DataTable({
            ...
            dom : 'tpi',
            columns: [
                { data: 'principal_cd', name: 'principal_cd', visible:true },
                { data: 'principal_nm', name: 'principal_nm' },
            ],
        });

        $(document).on('keyup', '#search_param',function(){ 
            tabelData.column('#principal_nm_table').search($(this).val()).draw();
        });
        ...
        $(document).on('click', '#ubah',function(){ 
            if (dataCd == null) {
                ...
                $('input[name=principal_cd]').val(rowData['principal_cd']).prop('readonly',true);
                $('input[name=principal_nm]').val(rowData['principal_nm']);
        ...
        /*cek kode*/
        $('input[name=principal_cd]').focusout(function(){
            var id          = $(this).val();
                ...
                $.getJSON( urlUpdate, function(data){
                    if (data['status'] == 'ok') {
                        swal({
                            ...
                            $('input[name=principal_cd]').val('');
                            $('input[name=principal_cd]').focus();
                            swal.close();
                        });
                    }
                });
    ...
    function reset(type) {
        ...
        $('#bagian-tabel').show();      
        $('#bagian-form').hide(); 
        $('input[name=principal_cd]').val('').prop('readonly',false);
        $('input[name=principal_nm]').val('');
        $('.card-title').text('Data Principal');       
    }
</script>
@endpush