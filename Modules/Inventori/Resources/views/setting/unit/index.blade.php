                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group form-group-float">
                            {{-- <label class="form-group-float-label is-visible">Nama Satuan</label> --}}
                            <input name="search_param" id="search_param" placeholder="Pencarian Nama Satuan" class="form-control" data-fouc />
                        </div>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-single-select datatable-pagination" id="tabel-data" width="100%">
                        <thead>
                            <tr>
                                <th>Kode Satuan</th>
                                <th id="unit_nm_table">Nama Satuan</th>
                            </tr>
                        </thead>
                        <tbody>
                        </tbody>
            ...
            <div id="bagian-form">
                <form class="form-validate-jquery" id="form-isian" action="#">
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group form-group-float">
                                <label class="form-group-float-label is-visible">Kode Satuan</label>
                                <input type="text" name="unit_cd" class="form-control text-uppercase" required="" placeholder="" aria-invalid="false">
                            </div>
                        </div>
                        <div class="col-md-8">
                            <div class="form-group form-group-float">
                                <label class="form-group-float-label is-visible">Nama Satuan</label>
                                <input type="text" name="unit_nm" class="form-control" required="" placeholder="" aria-invalid="false">
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    var baseUrl     = "{{ url('inventori/setting/satuan/') }}";
    ...
    $(document).ready(function(){
        ...
        tabelData = $('#tabel-data').DataTable({
            ...
            columns: [
                ...
                { data: 'unit_nm', name: 'unit_nm' },
            ],
        });

        $(document).on('keyup', '#search_param',function(){ 
            tabelData.column('#unit_nm_table').search($(this).val()).draw();
        });
        ...
        $(document).on('click', '#ubah',function(){ 
            if (dataCd == null) {
                ...
                $('input[name=unit_cd]').val(rowData['unit_cd']).prop('readonly',true);
                $('input[name=unit_nm]').val(rowData['unit_nm']);
        ...
        /*cek kode*/
        $('input[name=unit_cd]').focusout(function(){
            var id          = $(this).val();
                ...
                $.getJSON( urlUpdate, function(data){
                    if (data['status'] == 'ok') {
                        swal({
                            ...
                            $('input[name=unit_cd]').val('');
                            $('input[name=unit_cd]').focus();
                            swal.close();
                        });
                    }
                });
    ...
    function reset(type) {
        ...
        $('#bagian-tabel').show();      
        $('#bagian-form').hide(); 
        $('input[name=unit_cd]').val('').prop('readonly',false);
        $('input[name=unit_nm]').val('');
        $('.card-title').text('Data Satuan');       
    }
</script>
@endpush